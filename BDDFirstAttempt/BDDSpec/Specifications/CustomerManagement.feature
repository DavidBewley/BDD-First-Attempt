...
Scenario: User wants to add a new customer to the list of current customers
	Given the user has access to the system
	When the user requests the creation of a new customer with the following details FirstName:"John" 
	Surname:"Smith" EmailAddress"John.Smith@email.co.uk"
	Then the customer is added to the list of current customers

Scenario: User wants to edit a current customers details
	Given the user has access to the system
	When the user requests an edit of a customer with the Id 1 to the following details FirstName:"Jane" 
	Surname:"Smith" EmailAddress"Jane.Smith@email.co.uk"
